...
- 用户问“为什么坏了”。
- 多次修复失败，需要重新建立诊断路径。
- 例如：“帮我修这个报错”、“这个测试为什么挂了”、“帮我查这个接口为什么变慢了”。

不适用场景包括：

- 普通功能实现，例如“实现一个新接口”。
- 常规重构，例如“重构这个模块”。
- 没有失败现象的代码修改，例如“加一个配置项”。
...
如果用户已经给出明确 root cause 或 fix，可以简化流程：先确认用户判断和证据，做最小验证，再展示简短 diagnosis summary / fix strategy。修改代码前仍必须等待用户确认。
...
- 读取相关文件。
- 运行安全的本地验证命令：优先只读或局部命令，不访问外部网络，不写入敏感路径，不执行 destructive 命令；有副作用或边界不清时先确认。
- 缩小失败范围。
