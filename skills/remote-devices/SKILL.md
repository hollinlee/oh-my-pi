...
## Remote command output is returned in the final tool result. SSH heartbeat, timeout diagnostics, and bounded stdout/stderr capture remain active; if the command is long-running, rely on the explicit timeout budget and final diagnostics.
