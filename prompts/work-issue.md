...
- 每个 issue 开始前 fetch 远端 base branch，并在干净工作树上 fast-forward-only 同步；从最新 base 创建 conventional branch，确保每个 issue 独立 branch 和 PR。
- PR 创建前重新 fetch 并校验 base/head SHA；base drift、behind 或同步冲突时停止，不自动 rebase/force-push。
...
- merge 后切回并同步 `main`，确认本地与远端 base SHA 一致且工作树干净，再处理下一个 issue。
