---
description: 自动处理 GitHub PR review，验证并在 ready 后合并
...
- 读取并分类 review comments；处理不改变 scope 的 must-fix 和 question。
- 处理后运行相关验证，自动 commit/push review fixes。
- 回复并 resolve 已处理 threads。
- required checks 通过后直接进入 merge gate。
- review ready 后按 `references/merge.md` 检查 authoritative blockers；无 blocker 时自动 squash merge + delete branch。
- 只有触发 human decision gate、仍有 blocking review comment 或 merge gate 未通过时停止，并展示剩余风险。
