...
import { usageIntakePath } from "./usage/intake.ts";
import { usageStateDir } from "./usage/paths.ts";
import { USAGE_SCHEMA_VERSION } from "./usage/store.ts";
...
export function checkUsageHealth(): DoctorCheck[] {
  ...
  if (!dbStat) {
    ...
    try {
      ...
      checks.push({ severity: "fail", label: "usage ledger schema unreadable", detail: truncateDetail((error as Error).message) });
    }
  }
