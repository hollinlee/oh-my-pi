...
test("single-file happy path uploads, polls, materializes Markdown, and never persists the token", async () => {
  ...
  const mockFetch = (async (input: string | URL | Request, init: RequestInit = {}) => {
    ...
    if (url === "https://upload.test/file") {
      for await (const chunk of init.body as unknown as AsyncIterable<Buffer | Uint8Array>) uploadedBytes += Buffer.from(chunk).byteLength;
    ...
    if (url === "https://download.test/result.zip") return new Response(zip, { status: 200 });
