...
默认收起显示当前阶段、已耗时和 realtime status；`Alt+O` 可展开或收起历史 trace，当前状态行位置保持不变。即使模型尚未发布 canonical phase，也会显示带计时的 `Working` fallback，避免阶段区为空。环境 footer 仍位于输入框下方。对于可见的 canonical 阶段，错误会结束阶段并显示失败符号，agent 正常结束时显示完成符号；新用户输入会清除上一轮结果。
